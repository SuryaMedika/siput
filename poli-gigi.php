...
<?php
$id = abs((int)$_GET['id']);
$get = mysql_query("SELECT * FROM pasien WHERE pasien_stage = 'Poli Gigi' AND pasien_id='$id'");
...
$query = "SELECT * FROM pasien WHERE pasien_stage = 'Poli Gigi' ORDER BY pasien_update ASC";
...
?>
                ...
                <tr>
                    <td>
                        <a href="poli-gigi.php?aksi=view&id=<?php echo $data['pasien_id']; ?>" class="title">
    ...
    if($_POST['simpan-pasien']){
      ...
      if($_POST['stage'] == 'Apotek' && $anamnese && $diagnose && $treatment){
        ...
        if($update && $insert){
          ...
          $message_error = "ERROR: Gagal menambahkan kunjungan.";
        }

      }else{
        $message_error = "ERROR: Pilih langkah selanjutnya. Dan jangan lupa isi Anamnesa, Diagnosa Dan Tindakan pasien.";
      }
...
?>
    ...
    <div class="box-">
        <h1>
            Poli Gigi <?php echo "&raquo; <strong>".$dataGet['pasien_number']."</strong>"; ?>
        </h1>
    </div>
